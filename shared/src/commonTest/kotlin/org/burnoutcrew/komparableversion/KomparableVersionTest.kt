...
class KomparableVersionTest {
    ...
    private fun checkVersionsOrder(versions: Array<String>) {
        val c: Array<KomparableVersion> = versions.map { newComparable(it) }.toTypedArray()
        for (i in 1 until versions.size) {
            val low: KomparableVersion = c[i - 1]
            for (j in i until versions.size) {
                val high: KomparableVersion = c[j]
                assertTrue(low < high, "expected $low < $high")
                assertTrue(high > low, "expected $high > $low")
            }
        }
    }

    private fun checkVersionsEqual(v1: String, v2: String) {
        val c1: KomparableVersion = newComparable(v1)
        val c2: KomparableVersion = newComparable(v2)
    ...
    private fun checkVersionsOrder(v1: String, v2: String) {
        val c1: KomparableVersion = newComparable(v1)
        val c2: KomparableVersion = newComparable(v2)
        assertTrue(c1 < c2, "expected $v1 < $v2")
        assertTrue(c2 > c1, "expected $v2 > $v1")
    }
    ...
    fun testMng6572() {
        ...
        checkVersionsOrder(a, c)
        assertFails { checkVersionsOrder(c, d) }
        assertFails { checkVersionsOrder(b, d) }
        assertFails { checkVersionsOrder(a, d) }
    }
    ...
    fun testVersionZeroEqualWithLeadingZeroes() {
        ...
        checkVersionsArrayEqual(arr)
        assertFails { checkVersionsArrayEqual(arrayOf("0000000000000000000")) }
    }
    ...
    @Test
    fun testReuse() {
        val c1 = KomparableVersion("1")
        c1.parseVersion("2")
        val c2: Comparable<*> = newComparable("2")
